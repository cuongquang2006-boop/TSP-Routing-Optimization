...
private:
    ...
    void drawEdgeWithGlow(QPainter &painter, const QPointF &from, const QPointF &to,
                         const QColor &color, qreal progress);
    QColor getEdgeColor(bool isInTour, bool isHighlighted) const;

    int getEdgeWidth(bool isHighlighted) const;
    void generateSweepParticles(const QPointF &from, const QPointF &to, qreal progress);
